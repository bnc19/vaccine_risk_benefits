...
all_risk$v_deaths = c(0,3)
all_risk$p_deaths = all_risk$v_deaths/all_risk$doses
...
dalys_averted = final_results %>%  
  ...
  mutate(color = ifelse(mean <0, "Net risk from vaccination", "Net benefit from vaccination")) %>% 
  ggplot(aes(x = mean, y = age)) +
  geom_bar(stat = "identity", aes(fill = color)) +
  geom_errorbar(aes(xmin = low, xmax = high),  width =0.25) +
  facet_wrap(~ scenario,  ncol = 1) +
  my_theme +
  scale_fill_manual(values= c( "Net risk from vaccination" = "#FF6F61",
                               "Net benefit from vaccination" = "#1B3B6F")) +
  geom_vline(xintercept = 0, linetype = 2) +
  xlim(c(-55,55)) +
  theme(legend.position = "bottom") +
  labs(y = "Age group", x = "Difference in DALYs per 10,000 vaccinated")


dalys_averted

ggsave(dalys_averted, 
       filename = "output/difference_in_dalys_3_deaths.jpg",
       units = "cm",
       height = 10,
       width = 10
)
